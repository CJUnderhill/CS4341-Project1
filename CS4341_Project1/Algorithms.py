...
def beam(neighbors, queue, graph_data, params):
    ...
    newQueue.extend(tempQueue)
    pathLength = len(newQueue[0]['path'])
    
    # Check newQueue to see if all paths are of equal length
    for n in newQueue:
        if not len(n['path']) == pathLength:
            return newQueue
    # If the level is finished, shrink the queue
    newQueue.sort(key=leastCost)
    newQueue = newQueue[:2]
    # Sort lexographically (left-right)
    newQueue.sort(key=lex)
